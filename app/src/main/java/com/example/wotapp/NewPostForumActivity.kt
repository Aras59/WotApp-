...
class NewPostForumActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        categorySpinnerAdapter = ArrayAdapter.createFromResource(this,R.array.category,R.layout.spinner_category)
    ...
    private fun createPost(data: HashMap<String,String>,category:String){
        val random = kotlin.math.abs(
            Random(
                LocalDateTime.now().toEpochSecond(ZoneOffset.UTC)
            ).nextLong()
        )
        firestore.collection("forumPosts").document(category)
            .collection("post").document(random.toString()).set(data)
            .addOnSuccessListener { Toast.makeText(baseContext, "Post created!.",
