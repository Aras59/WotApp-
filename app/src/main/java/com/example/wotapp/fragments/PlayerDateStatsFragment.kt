...
class PlayerDateStatsFragment : Fragment() {
    ...
    private lateinit var avgAssistView: TextView
    ...
    override fun onCreateView(
        ...
        avgAssistView = view.findViewById(R.id.avgAsistView)
        ...
        fireStore.collection("followingusers")
            ...
            .addOnCompleteListener {
                ...
                if (task.isSuccessful && task.result.data != null) {
                    val followingDate = task.result.data!!["followingdate"].toString()
                    val datesBetweenTodayAndFollow = getDates(followingDate,formatted)
                    if(datesBetweenTodayAndFollow.size == 1){
                        datesBetweenTodayAndFollow.clear()
                        datesBetweenTodayAndFollow.add("This player was not tracked by date yet")
                    }
                    val datesAdapter = activity?.let { ArrayAdapter(it,R.layout.spinner_list
    ...
    private fun settingStatsOnView(player: Player, stats: DocumentSnapshot) {
        ...
        if( battles.toDouble() != 0.0) {
            ...
            val avgAssist = ((player.statistics.all.avg_damage_assisted * player.statistics.all.battles - stats.get("avg_damage_assisted")
                .toString().toDouble() * stats.get("battles").toString().toInt())/battles.toDouble()).toString()

            avgAssistView.text = " "+ BigDecimal(avgAssist.toDouble()).setScale(2, RoundingMode.HALF_EVEN).toString()+" "
